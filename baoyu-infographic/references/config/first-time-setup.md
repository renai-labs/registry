...
Use a single `question` with multiple questions (the runtime auto-adds an "Other" option):
...
`preferred_image_backend: auto` is the baked-in default — first-time setup never asks about it. The `## Image Generation` rule in SKILL.md then dispatches to ren's `image-generation` skill as the backend.
