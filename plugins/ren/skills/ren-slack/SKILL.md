...
See [[ren-scope]]. Slack is **org-level**: the install and its channel mappings are keyed to the caller's org regardless of any `--scope user` flag. The project a channel maps to can still live in a user-private pod.
...
```
ren slack status                       # { hasInstallation, installations[] }
ren slack install --output json        # → { url } — user opens it, grants workspace consent
ren slack channels list --output json  # live channels + current mapping (id, name, projectId, …)
ren slack uninstall                    # disconnect the org's workspace
...
```
mcp__ren__slack_status         { }
mcp__ren__slack_install        { }                                  # → { url }
mcp__ren__slack_channel_list   { }
mcp__ren__slack_channel_set    { "path": { "channelId": "C…" },
                                 "body": { "projectId": "prj_…", "defaultProjectAgentId": "pra_…",
                                           "fallbackSenderUserId": "usr_…" } }
mcp__ren__slack_channel_unset  { "path": { "channelId": "C…" } }
mcp__ren__slack_uninstall      { }
```
