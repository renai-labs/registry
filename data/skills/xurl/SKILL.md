...
Required tool: the `xurl` CLI (https://github.com/xdevplatform/xurl). Pick one install method on Linux or macOS:
...
`xurl` persists OAuth state in `~/.xurl` (YAML). OAuth 2.0 tokens auto-refresh. Each registered app has isolated tokens.
...
After this, the agent can use any command below without further setup.
...
App credential registration and credential rotation must be done by the user manually, outside the agent session. After credentials are registered, the user authenticates with `xurl auth oauth2` — also outside the agent session. Tokens persist to `~/.xurl` in YAML. Each app has isolated tokens.
...
3. If auth is missing entirely, stop and direct the user to "One-Time User Setup" — do NOT attempt to register apps or pass secrets yourself.
