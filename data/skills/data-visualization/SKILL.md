...
Use this skill as the implicit orchestrator for visualization work. Classify the task, choose the smallest useful approach, and decide the route before doing deep chart, renderer, testing, accessibility, or export work. The Routing Matrix below names the technique families; the bundled `references/foundations/` carry the depth for each.
...
7. Choose the narrowest applicable approach from the Routing Matrix below. If the request spans several visual layers, read `references/foundations/embedded-visualization-self-use.md`, inventory the layers, assign owners, and handle each substantial layer in its own pass.
...
- For implementation: pick the narrowest applicable approach from the Routing Matrix, then state renderer ownership, coordinate/data encoding, interaction states, fallback/render-ready behavior, instance-count assumption, performance risks, and tests before editing.
