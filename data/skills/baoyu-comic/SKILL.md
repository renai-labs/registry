...
When this skill needs to ask the user something, prefer the agent runtime's built-in user-input tool (e.g., `question`). Batch multiple questions into a single call when the tool supports it; otherwise ask them one at a time in priority order. If no such tool exists, emit a numbered plain-text message and have the user reply with the chosen number.

## Image Generation

This skill renders images by calling the **`image-generation` skill** (the ren primitive). Read its `SKILL.md` for the exact CLI; the typical invocation is `python3 scripts/cli.py generate "<prompt>" -o <out.png> [--aspect 3:4]` from that skill's directory.

**Prompt file requirement (hard)**: write each image's full, final prompt to a standalone file under `prompts/` (naming: `NN-{type}-[slug].md`) BEFORE invoking the image-generation skill. The prompt file is the reproducibility record — it lets you regenerate any panel by re-reading the file and re-running the backend.

If `EXTEND.md` sets `preferred_image_backend` to a value other than `image-generation`, treat that as a per-user override (the user has wired up an alternative backend). Otherwise, default to the `image-generation` skill.
...
**Backend**: the `image-generation` skill (ren primitive). Read its `SKILL.md` for the current CLI flags; this skill issues prompts via `python3 scripts/cli.py generate ... -o <path>` from that skill's directory.
...
- **Common one-line edits**:
  - `preferred_image_backend: image-generation` — default (ren's image-generation skill).
  - `preferred_image_backend: ask` — confirm backend every run (only useful if you've wired up an alternative skill).
