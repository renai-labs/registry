...
2. Use the `question` tool to collect preferences (see `config/first-time-setup.md`)
...
**If directory exists**, use the `question` tool:
...
**Strategy A: Using `--ref` parameter** (via the image-generation skill)
