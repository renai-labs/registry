...
metadata:
  ...
  homepage: https://www.hyperframes.ai
  tags:
    - design
  requiredCredentials:
    - name: HEYGEN_API_KEY
      description: Required for HeyGen media search, catalog retrieval, Starfish TTS, and other HeyGen-backed providers; offline fallbacks remain available without it.
    - name: ELEVENLABS_API_KEY
      description: Required only when ElevenLabs is selected as the TTS fallback provider.
    - name: GEMINI_API_KEY
      description: Required only when Google Lyria is selected for generated background music; GOOGLE_API_KEY is also accepted upstream.
---
