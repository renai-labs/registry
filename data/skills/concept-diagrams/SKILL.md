...
description: >-
  ...
  using a unified educational visual language with 9 semantic color ramps,
  sentence-case typography, and automatic dark mode. Best suited for educational
  and non-software visuals — physics setups, chemistry mechanisms, math curves,
  physical objects (aircraft, turbines, smartphones, mechanical watches),
  anatomy, floor plans, cross-sections, narrative journeys (lifecycle of X,
  process of Y), hub-spoke system integrations (smart city, IoT), and exploded
  layer views. If a more specialized skill exists for the subject (dedicated
  software/cloud architecture, hand-drawn sketches, animated explainers, etc.),
  prefer that — otherwise this skill can also serve as a general-purpose SVG
...
3. Read `templates/template.html` (relative to this skill's directory) and use it as the wrapper — paste your SVG where the template says `<!-- PASTE SVG HERE -->`.
4. Write the filled-in HTML to a standalone `.html` file (for example `~/my-diagram.html` or `./my-diagram.html`).
...
For physical, infrastructure, and dashboard diagrams, read the matching reference file (under `references/`) before generating — each one provides ready-made CSS classes and shape primitives.
...
1. Read `templates/template.html` from this skill's directory.
2. Replace the placeholder comments with the diagram title, optional subtitle, and your SVG:
   - `<!-- DIAGRAM TITLE HERE -->` → e.g. `SN2 reaction mechanism`
   - `<!-- OPTIONAL SUBTITLE HERE -->` → e.g. `Bimolecular nucleophilic substitution`
   - `<!-- PASTE SVG HERE -->` → the full SVG markup
3. Write the result to a user-chosen path (defaulting to `./<slug>.html`).
...
Read any example with the agent's Read tool — they're plain markdown files under `examples/` in this skill.
