...
Designs are sourced from VoltAgent/awesome-design-md.

## When to Use

- "build a page that looks like [brand]"
- "make it look like Stripe / Linear / Vercel"
- creating a landing page, marketing site, or dashboard in the style of a known brand
- mood-boarding a UI before custom design work

## Setup

No external dependencies. The agent reads the design template directly:

1. Pick a design from the [catalog](#design-catalog) below.
2. Read the matching file with the Read tool: `templates/<site>.md` (relative to this skill's directory).
3. Use the design tokens, type stack, and component specs from the template when generating HTML.

Each template starts with an **Implementation Notes** block that lists the Google Fonts CDN substitute and a paste-ready `<link>` tag for self-contained HTML output.
...
Save the result as a standalone `.html` file. The user can open it directly in a browser (`open page.html` on macOS, `xdg-open page.html` on Linux) — no server required. If you want to preview it yourself, read the file back and inspect the markup, or render via a local loopback server (bind to `127.0.0.1` only, ephemeral port).
