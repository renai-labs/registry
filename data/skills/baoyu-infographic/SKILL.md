...
When this skill needs to ask the user something, use opencode's `question` tool. Batch multiple questions into a single call when possible; otherwise ask them one at a time in priority order.
...
**Prompt file requirement (hard)**: write the image's full, final prompt to a standalone file under `prompts/` (naming: `NN-{type}-[slug].md`) BEFORE invoking the image-generation skill. The prompt file is the reproducibility record.

If `EXTEND.md` sets `preferred_image_backend` to a value other than `image-generation`, treat that as a per-user override. Otherwise, default to the `image-generation` skill.
...
- If `usage: direct` AND the chosen backend accepts reference images (via `--ref` if supported) → pass the file via the backend's ref parameter
...
| Not found | Ask the user with `question` (see `references/config/first-time-setup.md`) |
