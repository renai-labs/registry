...
| `preferred_image_backend` | string | `auto` | Image backend selection. `auto` = prefer runtime-native tool, fall back to the only installed backend, ask if multiple non-native are present. `ask` = always confirm on every run. `<backend-id>` (e.g., `image-generation`) = pin this backend when available; fall back to `auto` when it isn't. Absent = `auto`. |
...
| Value | Meaning |
|-------|---------|
| `image-generation` | ren's `image-generation` skill (default) |
| `ask` | confirm backend on every run |
...
preferred_image_backend: image-generation
