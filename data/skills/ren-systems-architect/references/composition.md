...
| **Telegram group** | @mention, `/agent` slash command, or a reply to one of the bot's messages                        | plain group chatter is ignored; a new non-reply message starts a fresh session with no history |
...
| **Linear**         | an agent session created or prompted on a mapped workspace                                        | only those two events; webhooks older than 60s are dropped                                   |
...
| Subagent      | borrow an integration another agent holds; you fold in the result     |
...
5. **Capability lives on another agent** → task it as a subagent, fold the result in. No new agent.
6. **Knowledge that must apply to every run of one outcome** → project instructions, not a longer
