...
- **Inbound (mapping).** The message arrives as an ordinary session turn and the reply streams back
  to the same thread. The agent needs no channel-specific code and **no MCP**.
- **Outbound (the channel MCP).** The agent initiates — posts, sends, reacts. Attached through the
  agent version's `mcpIds`, and **only the meta agent may hold these MCPs**; a worker agent hands its
  result back instead.

Facts common to all mappings:

- `defaultProjectAgentId` is the **`pra_` attachment id** from `ren projects agents list <prj_…>`,
  not the `agt_` agent id. The wrong one fails validation.
...
Attaching a channel MCP (`mcpIds` is full-replace — merge, don't overwrite):

```bash
ren mcps get-by-slug slack          # → { "id": "mcp_…" }
ren agents get <agt_…> > /tmp/agent.json     # splice the id into the existing mcpIds
ren agents versions create <agt_…> --body @/tmp/agent.json --release-notes "attach slack MCP"
```

Tools appear in the next session; no restart. Remove by repeating without the id.
...
ren slack channels set <channel-id> --project-id <prj_…> \
  --default-project-agent-id <pra_…> --fallback-sender-user-id <usr_…>
...
- Each project agent is exposed as a `/<agent_name>` slash command, synced automatically; `/agents`
  lists them, `/help` and `/link` are built in. Without a command, the default agent answers.
...
- Replies are platform-managed back into the same thread. Threading uses a `+token` Ren mints on its
  ...
  token address.
- Mailboxes are per project — never configured at agent, pod or org level.
...
- **Give the agent the code** — bind the repository to the project:
...
Only agent-session events reach Ren — assigning it on an issue, or prompting it in an existing
