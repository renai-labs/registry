# Ren integrations

## Native


| name                                                              | mcp_slugs | skill_slugs      | notes                                                                                                                                                                                                           |
| ----------------------------------------------------------------- | --------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Slack                                                             | slack     | ren-slack        | Native org integration — install via `ren slack <>` for channel→project routing; attach the registry MCP `slack` to agents that call Slack proactively (`auth: "none"`, no credential)                          |
| GitHub                                                            |           | ren-github       | Native org integration; not a registry MCP — connect via `ren github <>`                                                                                                                                        |
| Google Workspace (Gmail, Calendar, Drive, Contacts, Sheets, Docs) |           | google-workspace | Native integration - requires user to create an oauth app. Use `ren google <>` Read google-workspace skill for setup instructions                                                                               |
| Email                                                             | email     | ren-email        | Use when user wants to send emails to Ren agents to delegate tasks or if Ren agents want to proactively send email notifications to users. For managing user's mailbox, use gmail(google workspace integration) |


## CRM


| name                      | mcp_slugs              | skill_slugs | notes |
| ------------------------- | ---------------------- | ----------- | ----- |
| Zoho CRM (India)          | zoho-crm-india         |             |       |
| Zoho CRM (US)             | zoho-crm-us            |             |       |
| Apollo.io                 | apollo                 |             |       |
| Vibe Prospecting          | vibe-prospecting       |             |       |
| Outreach                  | outreach               |             |       |
| Attio                     | attio                  |             |       |
| Crossbeam                 | crossbeam              |             |       |
| Gainsight (Staircase AI)  | gainsight-staircase-ai |             |       |
| Common Room               | common-room            |             |       |
| Close                     | close                  |             |       |
| Day AI                    | day-ai                 |             |       |
| Clarify                   | clarify                |             |       |
| Sprouts Data Intelligence | sprouts                |             |       |


## Marketing


| name                             | mcp_slugs                | skill_slugs | notes |
| -------------------------------- | ------------------------ | ----------- | ----- |
| AppsFlyer                        | appsflyer                |             |       |
| Gamma                            | gamma                    |             |       |
| Ahrefs                           | ahrefs                   |             |       |
| WordPress.com                    | wordpress-com            |             |       |
| Windsor.ai                       | windsor-ai               |             |       |
| Klaviyo                          | klaviyo                  |             |       |
| Wix                              | wix                      |             |       |
| GoDaddy                          | godaddy                  |             |       |
| Supermetrics Marketing Analytics | supermetrics             |             |       |
| Jotform                          | jotform                  |             |       |
| Adobe Experience Manager         | adobe-experience-manager |             |       |
| Adobe Marketing Agent            | adobe-marketing-agent    |             |       |
| AirOps                           | airops                   |             |       |
| Adobe Journey Optimizer          | adobe-journey-optimizer  |             |       |
| Bitly                            | bitly                    |             |       |
| Motion Creative Analytics        | motion                   |             |       |
| Peec AI                          | peec-ai                  |             |       |
| SurveyMonkey                     | surveymonkey             |             |       |
| MailerLite                       | mailerlite               |             |       |
| Local Falcon                     | local-falcon             |             |       |


## Support


| name     | mcp_slugs | skill_slugs | notes |
| -------- | --------- | ----------- | ----- |
| Intercom | intercom  |             |       |
| Pylon    | pylon     |             |       |
| Unthread | unthread  |             |       |
| Lorikeet | lorikeet  |             |       |


## Ecommerce


| name             | mcp_slugs        | skill_slugs | notes |
| ---------------- | ---------------- | ----------- | ----- |
| Swiggy Instamart | swiggy-instamart |             |       |
| Square           | square           |             |       |
| Ticket Tailor    | ticket-tailor    |             |       |


## Payments


| name                | mcp_slugs           | skill_slugs | notes |
| ------------------- | ------------------- | ----------- | ----- |
| Stripe              | stripe              |             |       |
| PayPal              | paypal              |             |       |
| Razorpay            | razorpay            |             |       |
| Cash App            | cash-app            |             |       |
| Airwallex Developer | airwallex-developer |             |       |
| GoCardless          | gocardless          |             |       |


## Finance


| name                            | mcp_slugs   | skill_slugs | notes |
| ------------------------------- | ----------- | ----------- | ----- |
| Ramp                            | ramp        |             |       |
| Brex                            | brex        |             |       |
| Ramp Data                       | ramp-data   |             |       |
| Mercury                         | mercury     |             |       |
| Chronograph                     | chronograph |             |       |
| Tropic                          | tropic      |             |       |
| Rillet                          | rillet      |             |       |
| Campfire                        | campfire    |             |       |
| Digits                          | digits      |             |       |
| Aiwyn Tax (formerly Column Tax) | aiwyn-tax   |             |       |


## Market data


| name                             | mcp_slugs                        | skill_slugs | notes |
| -------------------------------- | -------------------------------- | ----------- | ----- |
| S&P Global                       | s-p-global                       |             |       |
| FactSet AI-Ready Data            | factset                          |             |       |
| Morningstar                      | morningstar                      |             |       |
| Daloopa                          | daloopa                          |             |       |
| LSEG                             | london-stock-exchange-group      |             |       |
| Bigdata.com                      | bigdata                          |             |       |
| MT Newswires                     | mt-newswires                     |             |       |
| Fiscal.ai                        | fiscal-ai                        |             |       |
| Quartr                           | quartr                           |             |       |
| MSCI                             | msci                             |             |       |
| Yardi Virtuoso                   | yardi-virtuoso                   |             |       |
| IBISWorld                        | ibisworld                        |             |       |
| GovTribe                         | govtribe                         |             |       |
| Kindora Funder Discovery         | kindora-funder-discovery         |             |       |
| Clarity AI                       | clarity-ai                       |             |       |
| Verisk Underwriting Intelligence | verisk-underwriting-intelligence |             |       |


## Stock trading


| name            | mcp_slugs    | skill_slugs | notes |
| --------------- | ------------ | ----------- | ----- |
| Groww           | groww        |             |       |
| Kite by Zerodha | zerodha-kite |             |       |


## Crypto


| name       | mcp_slugs  | skill_slugs | notes |
| ---------- | ---------- | ----------- | ----- |
| Crypto.com | crypto-com |             |       |
| LunarCrush | lunarcrush |             |       |
| Blockscout | blockscout |             |       |
| CoinDesk   | coindesk   |             |       |


## Private markets


| name              | mcp_slugs    | skill_slugs | notes |
| ----------------- | ------------ | ----------- | ----- |
| PitchBook Premium | pitchbook    |             |       |
| CB Insights       | cb-insights  |             |       |
| Third Bridge      | third-bridge |             |       |
| Harmonic          | harmonic     |             |       |


## HR


| name           | mcp_slugs      | skill_slugs | notes |
| -------------- | -------------- | ----------- | ----- |
| Udemy Business | udemy-business |             |       |
| Metaview       | metaview       |             |       |
| Dice           | dice           |             |       |
| Gusto          | gusto          |             |       |
| ZipRecruiter   | ziprecruiter   |             |       |
| Workable       | workable       |             |       |


## Email


| name            | mcp_slugs       | skill_slugs | notes |
| --------------- | --------------- | ----------- | ----- |
| Superhuman Mail | superhuman-mail |             |       |


## Calendar


| name     | mcp_slugs | skill_slugs | notes |
| -------- | --------- | ----------- | ----- |
| Calendly | calendly  |             |       |


## Meetings


| name       | mcp_slugs  | skill_slugs | notes |
| ---------- | ---------- | ----------- | ----- |
| Attention  | attention  |             |       |
| Granola    | granola    |             |       |
| Fireflies  | fireflies  |             |       |
| Fathom     | fathom     |             |       |
| Fellow.ai  | fellow-ai  |             |       |
| Otter.ai   | otter-ai   |             |       |
| Circleback | circleback |             |       |
| Krisp      | krisp      |             |       |
| Grain      | grain      |             |       |
| Quo        | quo        |             |       |
| Sybill     | sybill     |             |       |
| Zocks      | zocks      |             |       |


## Docs


| name                   | mcp_slugs  | skill_slugs | notes |
| ---------------------- | ---------- | ----------- | ----- |
| Notion                 | notion     |             |       |
| Egnyte                 | egnyte     |             |       |
| PDF Viewer             | pdf-viewer |             |       |
| Guru                   | guru       |             |       |
| Midpage Legal Research | midpage    |             |       |
| Mem                    | mem        |             |       |
| Craft                  | craft      |             |       |
| SignNow                | signnow    |             |       |
| Lumin                  | lumin      |             |       |
| LILT                   | lilt       |             |       |


## Project management


| name           | mcp_slugs | skill_slugs | notes |
| -------------- | --------- | ----------- | ----- |
| Atlassian Rovo | atlassian |             |       |
| Linear         | linear    |             |       |
| monday.com     | monday    |             |       |
| ClickUp        | clickup   |             |       |
| Todoist        | todoist   |             |       |


## Design


| name                  | mcp_slugs             | skill_slugs | notes |
| --------------------- | --------------------- | ----------- | ----- |
| Mobbin                | mobbin                |             |       |
| VEED                  | veed                  |             |       |
| Canva                 | canva                 |             |       |
| Miro                  | miro                  |             |       |
| Excalidraw            | excalidraw-app-demo   |             |       |
| Lucid                 | lucid                 |             |       |
| Mermaid Chart         | mermaid-chart         |             |       |
| tldraw                | tldraw                |             |       |
| Play Sheet Music      | play-sheet-music      |             |       |
| Autodesk Product Help | autodesk-product-help |             |       |
| Magic Patterns        | magic-patterns        |             |       |
| Cloudinary            | cloudinary            |             |       |
| Splice                | splice                |             |       |
| Eraser                | eraser                |             |       |


## Engineering


| name                          | mcp_slugs         | skill_slugs | notes |
| ----------------------------- | ----------------- | ----------- | ----- |
| Cypress                       | cypress           |             |       |
| Datadog                       | datadog           |             |       |
| GitLab                        | gitlab            |             |       |
| Infobip                       | infobip           |             |       |
| Langfuse                      | langfuse          |             |       |
| LogRocket                     | logrocket         |             |       |
| New Relic                     | new-relic         |             |       |
| Prisma                        | prisma            |             |       |
| Railway                       | railway           |             |       |
| Rootly                        | rootly            |             |       |
| Statsig                       | statsig           |             |       |
| Trunk                         | trunk             |             |       |
| Microsoft Learn               | microsoft-learn   |             |       |
| Vercel                        | vercel            |             |       |
| Sentry                        | sentry            |             |       |
| Supabase                      | supabase          |             |       |
| Context7                      | context7          |             |       |
| Postman                       | postman           |             |       |
| Hugging Face                  | hugging-face      |             |       |
| Webflow                       | webflow           |             |       |
| Cloudflare Developer Platform | cloudflare        |             |       |
| Netlify                       | netlify           |             |       |
| incident.io                   | incident-io       |             |       |
| Base44                        | base44            |             |       |
| Three.js 3D Viewer            | threejs-3d-viewer |             |       |
| Sanity                        | sanity            |             |       |
| Jam                           | jam               |             |       |
| Honeycomb                     | honeycomb         |             |       |
| Clerk                         | clerk             |             |       |
| PlanetScale                   | planetscale       |             |       |
| Twilio                        | twilio            |             |       |
| Stytch                        | stytch            |             |       |
| pg-aiguide                    | pg-aiguide        |             |       |
| GraphOS MCP Tools             | graphos-tools     |             |       |


## Analytics


| name             | mcp_slugs        | skill_slugs | notes |
| ---------------- | ---------------- | ----------- | ----- |
| Keboola          | keboola          |             |       |
| Airtable         | airtable         |             |       |
| Mixpanel         | mixpanel         |             |       |
| PostHog          | posthog          |             |       |
| Omni Analytics   | omni-analytics   |             |       |
| CData Connect AI | cdata-connect-ai |             |       |
| Enterpret        | enterpret-wisdom |             |       |
| Coupler.io       | coupler-io       |             |       |
| MotherDuck       | motherduck       |             |       |
| Contentsquare    | contentsquare    |             |       |
| Amplitude        | amplitude        |             |       |
| Orion by Gravity | orion            |             |       |
| Polar Analytics  | polar-analytics  |             |       |
| MoSPI            | mospi            |             |       |
| Shapes           | shapes           |             |       |


## Automation


| name   | mcp_slugs | skill_slugs | notes |
| ------ | --------- | ----------- | ----- |
| Zapier | zapier    |             |       |
| Make   | make      |             |       |
| IFTTT  | ifttt     |             |       |
| Jentic | jentic    |             |       |


## Research


| name            | mcp_slugs       | skill_slugs | notes |
| --------------- | --------------- | ----------- | ----- |
| Apify           | apify           |             |       |
| Consensus       | consensus       |             |       |
| Scholar Gateway | scholar-gateway |             |       |
| Open Targets    | open-targets    |             |       |
| Exa             | exa             |             |       |
| BioRender       | biorender       |             |       |
| Tavily          | tavily          |             |       |
| Synapse.org     | synapse-org     |             |       |
| Scite           | scite           |             |       |
| Medidata        | medidata        |             |       |
| AdisInsight     | adisinsight     |             |       |
| Ketryx          | ketryx          |             |       |


## Lifestyle


| name                          | mcp_slugs             | skill_slugs | notes |
| ----------------------------- | --------------------- | ----------- | ----- |
| Swiggy Dineout                | swiggy-dineout        |             |       |
| Swiggy Food                   | swiggy-food           |             |       |
| Kiwi.com                      | kiwi-com              |             |       |
| Tripadvisor                   | tripadvisor           |             |       |
| Trivago                       | trivago               |             |       |
| Resy                          | resy                  |             |       |
| AllTrails                     | alltrails             |             |       |
| Viator                        | viator                |             |       |
| Taskrabbit Booking Assistance | taskrabbit            |             |       |
| Melon                         | melon                 |             |       |
| Fever Event Discovery         | fever-event-discovery |             |       |


