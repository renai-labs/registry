...
4. **File-store hints.** File stores are mutable project filesystems: the agent can read user-supplied inputs and create, update, move, or delete output files when the task authorizes it. Where the source uses local input/output paths ("download dataset.csv first", "write the report to `./output/`", "place your PDFs in `./input/`"), flag it: `_In ren, keep these files in a file store attached to the project; it is a mutable persistent mount — see [[ren-systems-architect]]._` Preserve meaningful relative paths; do not move files merely to adapt the skill.
5. **Memory-store hints.** Use a memory store for durable agent memory and structured state that should be recalled across sessions, not as a substitute for ordinary project files. Where the source uses a dedicated memory database or resumable agent-state record, flag it: `_In ren, persist this as agent memory in an attached memory store — see [[ren-systems-architect]]._`
...
| "First, `wget https://example.com/corpus.csv` into `./data/`."              | "First, the agent reads `corpus.csv` from the attached mutable file store. See [[ren-systems-architect]]." |
| "Write the generated report to `./output/report.md`."                       | "Write the report to `output/report.md` in the attached file store. See [[ren-systems-architect]]."  |
| "Claude will then summarize each row."                                      | "The agent then summarizes each row."                                                                 |
| "Remember the last processed record across sessions."                       | "Persist that agent state to the attached memory store. See [[ren-systems-architect]]."               |
