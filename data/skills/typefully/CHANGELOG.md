...
- **Ren platform port.** The skill now expects to run inside the Ren agent runtime with credentials managed by the Ren vault.
  - API key is read exclusively from the `TYPEFULLY_API_KEY` environment variable. No config files, no priority chain.
  - Frontmatter rewritten to Ren's flat schema (`name`, `description`, `version`, `license`, `author`, `source`).

### Removed

- `setup`, `config:show`, and `config:set-default` commands and all interactive prompts.
- Config-file plumbing (`~/.config/typefully/config.json`, `./.typefully/config.json`) and the env → local → global API key priority chain.
- Persisted "default social set" concept. Every command now requires `social_set_id` positionally or via `--social-set-id`. The `--use-default` flag is gone.
