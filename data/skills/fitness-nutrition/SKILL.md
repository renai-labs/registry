...
source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/health/fitness-nutrition
---
...
Required tools: `curl`, `python3` (stdlib only — no pip).

Optional environment variable:

- `USDA_API_KEY` — USDA FoodData Central key. Free signup at https://fdc.nal.usda.gov/api-key-signup/. Without it, calls fall back to `DEMO_KEY` (30 requests/hour). With a free key, you get 1,000 requests/hour.

Set it once per shell session before running USDA lookups:

```bash
export USDA_API_KEY="your-key-here"
```

The scripts and curl snippets below pick this up automatically. wger needs no auth at all.
