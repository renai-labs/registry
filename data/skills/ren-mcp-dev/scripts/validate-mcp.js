    ...
    if (authServer) {
      ...
      if (as.ok) {
        out.authorizationServerMeta = {
          ...
          token_endpoint_auth_methods_supported: as.body.token_endpoint_auth_methods_supported,
        }
      }
    }
  }
...
async function validate(opts) {
  ...
  report.checks.https = {
    ...
    detail: parsed.protocol === "https:" ? "remote HTTPS endpoint" : `Ren requires https; got ${parsed.protocol}`,
  }
  ...
  try {
    ...
    if (init.res.status === 401 || init.res.status === 403) {
      ...
      const hasDiscovery = !!report.oauth.protectedResource || !!wwwAuth
  ...
  if (needsToken) {
    report.checks.tools = {
      ok: null,
      detail: "skipped - server is protected; pass --token to list tools (discovery validated above)",
    }
  } else {
    try {
      await rpc(
  ...
  if (opts.auth) {
    ...
    if (opts.auth === "none" && report.protected) ((ok = false), (detail = "declared none but server requires auth"))
...
function printHuman(report) {
  ...
  if (report.oauth) {
    ...
    if (report.oauth.authorizationServers)
      console.log(`    authorization_servers: ${report.oauth.authorizationServers.join(", ") || "(none)"}`)
    if (report.oauth.scopes) console.log(`    scopes: ${report.oauth.scopes.join(", ") || "(none)"}`)
  ...
  if (report.drift) {
    ...
    if (report.drift.tools) {
      ...
      console.log(`    ${mark(missing.length === 0)} tools missing on server: ${missing.join(", ") || "(none)"}`)
      console.log(`    — tools not in registry entry: ${extra.join(", ") || "(none)"}`)
    }
  }

  console.log("=".repeat(56))
  console.log(report.compatible ? "  RESULT: ✓ Ren-compatible" : "  RESULT: ✗ NOT Ren-compatible")
  console.log("")
}
