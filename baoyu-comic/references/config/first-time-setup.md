...
Use a single `question` call covering all of the prompts (the tool auto-adds an "Other" option):
...
`preferred_image_backend: auto` is the baked-in default — first-time setup does not ask about it. The `## Image Generation Tools` rule in SKILL.md then picks ren's `image-generation` skill as the backend.
